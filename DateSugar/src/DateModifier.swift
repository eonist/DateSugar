...
public class DateModifier {
    ...
    * ## EXAMPLES: NSDate().offsetByDays(-3)//an NSDate instance that is 3 days ago
    */
   public static func offsetByDays(_ date: Date, days: Int) -> Date? {
   ...
   public static func offsetByMonths(_ date: Date, months: Int) -> Date? {
      let calendar = Calendar.current
      let newDate: Date? = calendar.date(byAdding: .month, value: months, to: date)
      return newDate
   }
   public static func offsetByYears(_ date: Date, years: Int) -> Date? {
      let calendar = Calendar.current
      let newDate: Date? = calendar.date(byAdding: .year, value: years, to: date)
      return newDate
   }
   public static func offsetByHours(_ date: Date, hours: Int) -> Date? {
